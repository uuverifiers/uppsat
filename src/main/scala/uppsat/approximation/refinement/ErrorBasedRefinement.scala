...
/** Error based refinement strategy uses a measure of error to determine which
  * precisions need to be refined */
trait ErrorBasedRefinementStrategy extends PostOrderRefinement {
  ...
  /** Computes a (double) error value for a node. */
  ...
  override def satRefine(ast : AST,
                         decodedModel : Model,
                         failedModel : Model,
    ...
    val errorRatios =
      AST.postVisit(ast, accu, nodeError(decodedModel)(failedModel))
    val sortedErrRatios =
      errorRatios.toList.sortWith((x, y) => cmpErrors(x._2, y._2))

    // TODO: Assertions
    val k =
      math.ceil(fractionToRefine * sortedErrRatios.length).toInt

    val relevant =
      Toolbox.booleanComparisonOfModels(ast, decodedModel, failedModel)
    val toRefine =
      sortedErrRatios.filter( x => relevant.contains(x._1)).map(_._1)
    ...
    for (node <-
         toRefine.filter{
           x =>
           precisionOrdering.lt(newPMap(x.label), pmap.pOrd.maximalPrecision)
         }.take(k)) {
