...
case class BooleanTheoryException(msg : String)
    extends Exception("Boolean Theory Exception: " + msg)
...
object BooleanTheory extends Theory {
  ...
  class BooleanFunctionSymbol(val name :  String,
  ...
  class BooleanBinaryFunctionSymbol(override val name :  String)
      extends BooleanFunctionSymbol(name,
                                    List(BooleanSort, BooleanSort),
                                    BooleanSort) {
  }

  class BooleanUnaryFunctionSymbol(override val name :  String)
      extends BooleanFunctionSymbol(name, List(BooleanSort), BooleanSort) {
  }

  class BooleanConstant(override val name :  String)
      extends BooleanFunctionSymbol(name, List(), BooleanSort) {
  }

  case class NaryConjunction(argCount : Int)
      extends BooleanFunctionSymbol("nary-conjunction",
                                    List.fill(argCount)(BooleanSort),
                                    BooleanSort)

  def naryConjunction(i : Int) = new NaryConjunction(i)

  case class NaryDisjunction(argCount : Int)
      extends BooleanFunctionSymbol("nary-disjunction",
                                    List.fill(argCount)(BooleanSort),
                                    BooleanSort)
  ...
  case object BoolConjunction
      extends BooleanBinaryFunctionSymbol("conjunction")
  case object BoolDisjunction
      extends BooleanBinaryFunctionSymbol("disjunction")
  case object BoolExclusiveDisjunction
      extends BooleanBinaryFunctionSymbol("exclusive-disjunction")
  case object BoolImplication
      extends BooleanBinaryFunctionSymbol("implication")
  case object BoolEquality
      extends BooleanBinaryFunctionSymbol("equality")
  case object BoolNegation
      extends BooleanUnaryFunctionSymbol("negation")
  ...
  val symbols = List(BoolTrue, BoolFalse, BoolConjunction, BoolDisjunction,
                     BoolExclusiveDisjunction, BoolNegation)
  ...
  def symbolToSMTLib(symbol : ConcreteFunctionSymbol)
    ...
    symbol match {
      ...
      case _ => {
        val msg = "Boolean translation of non-Boolean symbol: " + symbol
        throw new BooleanTheoryException(msg)
      }
    }
  }

  override def sortToSMTLib(sort : ConcreteSort)
                           (implicit translator
                                : Option[SMTTranslator] = None) = {
    sort match {
      case BooleanSort => "Bool"
      case _ => {
        val msg = "Boolean translation of non-Boolean sort: " + sort
        throw new BooleanTheoryException(msg)
      }
    }
  }
  ...
  def declarationToSMTLib(sym : ConcreteFunctionSymbol) : String = {
    sym match {
      case BoolVar(name) =>
        "(declare-fun " + name + " () " + sortToSMTLib(BooleanSort) + ")"
      case _ => {
        val msg = "Boolean Declaration of non-Boolean symbol: " + sym
        throw new BooleanTheoryException(msg)
      }
    }
  }
