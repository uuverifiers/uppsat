...
def hexToBitList(hex : String) = {
  ...
  protected def translateSpecConstant(c : SpecConstant) : uppsat.ast.AST = {
    c match {
      case c : NumConstant => {
        myEnv.theory match {
          case Some(uppsat.theory.FloatingPointTheory) => {
            val i = BigInt(c.numeral_.toString)
            uppsat.ast.Leaf(uppsat.theory.RealTheory.RealNumeral(i))
          }
          case Some(uppsat.theory.IntegerTheory) => {
            val i = BigInt(c.numeral_.toString)
            uppsat.ast.Leaf(uppsat.theory.IntegerTheory.IntLiteral(i))
          }

          case _ => {
            throw new SMTParserException("Strange NumConstant: " + c)
          }
        }
      }
    ...
    case c : HexConstant =>
      ...
      c.hexadecimal_ match {
        ...
        case other =>
          throw new Exception("Strange hexadecimal literal: " + other)
      }
...
//        (MyIntLit(c.binary_ substring (2, 2)), SMTInteger)
      case  c => {
        val msg = "Unknown SpecConstant: " + c + " (" + c.getClass +")"
  ...
  private def parse(cmd : Command) : Unit = cmd match {
    ...
    case cmd : SetLogicCommand => {
      ...
      asString(cmd.symbol_) match {
        ...
        case _ => {
          val msg = "unknown set-logic command : \n"  + asString(cmd.symbol_)
          throw new Exception(msg)
        }
      }
    }
    ...
    case cmd : FunctionDeclCommand => {
      ...
      cmd.mesorts_ match {
        ...
        case _ => {
          val msg = "Function Declaration with arguments unsupported"
          throw new SMTParserException(msg)
        }
      }
    }
    ...
    case cmd : ConstDeclCommand => {
     ...
     case cmd : FunctionDefCommand => {
       ...
       if (!cmd.listesortedvarc_.isEmpty) {
         val msg = "Function Definitions with arguments unsupported"
         throw new SMTParserException(msg)
       } else {
         ...
         val symbol =
           resType match {
             ...
             case RoundingModeSort =>
               new uppsat.theory.FloatingPointTheory.RMVar(name)
           }
    ...
    case cmd : CheckSatCommand => {
      ...
      val usingTheory =
        if (myEnv.theory.isDefined)
          ...
          // Defaulting to FP since our project is currently focused on this.
          uppsat.theory.FloatingPointTheory
...
//        else throw new SMTParserException("No theory defined")
    ...
    //////////////////////////////////////////////////////////////////////////
     ...
     case cmd : GetModelCommand => {
       myEnv.result match {
         ...
         case _ =>
           throw new SMTParserException("Get-Model Command with no model")
       }
     ...
     case cmd : GetInfoCommand =>
       cmd.annotattribute_ match {
         ...
         case ":interpolation-method" => {
           val msg = "Get-Info Interpolation-Method Command unsupported"
           throw new SMTParserException(msg)
         }
         case ":reason-unknown" => {
           val msg = "Get-Info Reason-Uknown Command unsupported"
           throw new SMTParserException(msg)
         }
         case other =>
           throw new SMTParserException("Get-Info " + other + " unsupported")
       }
  ...
  protected def translateSort(s : Sort) : uppsat.ast.Sort = {
    ...
    s match {
      case s : IdentSort => asString(s.identifier_) match {
        ...
        case "Float32" =>
          uppsat.theory.FloatingPointTheory.FPSortFactory(List(8, 24))
        case fpPattern(eBits, sBits) =>
          uppsat.theory.FloatingPointTheory.FPSortFactory(List(eBits.toInt,
                                                               sBits.toInt))
        case bvPattern(bits) =>
          uppsat.theory.BitVectorTheory.BVSortFactory(List(bits.toInt))
        case id => {
  ...
  sym match {
    ...
    case PlainSymbol("=>") => {
      ...
      uppsat.ast.AST(BoolImplication,
                     List(translateTerm(args(0)),
                          translateTerm(args(1))))
    }
    ...
    case PlainSymbol("xor") => {
      ...
      uppsat.ast.AST(BoolExclusiveDisjunction,
                     List(translateTerm(args(0)),
                          translateTerm(args(1))))
    }
    ...
    case PlainSymbol("fp.isZero") => {
      ...
      uppsat.ast.AST(FloatingPointTheory.FPIsZeroFactory(ta.symbol.sort),
                     List(ta))
    }
    ...
    case PlainSymbol("fp.eq") => {
      ...
      uppsat.ast.AST(FloatingPointTheory.FPFPEqualityFactory(ta1.symbol.sort,
                                                             ta2.symbol.sort),
                     List(ta1, ta2))
    }

    // We can't use syntactic sugar since first leaf might not be a
    // rounding-mode but rather a defined function
    case PlainSymbol("fp.add") => {
      ...
      (ta(0).symbol.sort, ta(1).symbol.sort, ta(2).symbol.sort) match {
        case (RoundingModeSort, fp1 : FPSort, fp2 : FPSort) if (fp1 == fp2) =>
          uppsat.ast.AST(FloatingPointTheory.FPAdditionFactory(fp1, fp1, fp1),
                         ta.toList)
        case (s1, s2, s3) => {
          val msg = "Wrong sorts for fp.add: " + ((s1, s2, s3))
          throw new SMTParserException(msg)
        }
      }
    }
    ...
    case PlainSymbol("fp.sub") => {
      ...
      (ta(0).symbol.sort, ta(1).symbol.sort, ta(2).symbol.sort) match {
        case (RoundingModeSort, fp1 : FPSort, fp2 : FPSort) if (fp1 == fp2) =>
          uppsat.ast.AST(FloatingPointTheory.FPSubstractionFactory(fp1,
                                                                   fp1,
                                                                   fp1),
                         ta.toList)
        case (s1, s2, s3) => {
          val msg = "Wrong sorts for fp.sub: " + ((s1, s2, s3))
          throw new SMTParserException(msg)
        }
      }
    }
    ...
    case PlainSymbol("fp.mul") => {
      ...
      (ta(0).symbol.sort, ta(1).symbol.sort, ta(2).symbol.sort) match {
        case (RoundingModeSort, fp1 : FPSort, fp2 : FPSort) if (fp1 == fp2) =>
          uppsat.ast.AST(FloatingPointTheory.FPMultiplicationFactory(fp1,
                                                                     fp1,
                                                                     fp1),
                         ta.toList)

        case (s1, s2, s3) => {
          val msg = "Wrong sorts for fp.mul: " + ((s1, s2, s3))
          throw new SMTParserException(msg)
        }
      }
    }
    ...
    case PlainSymbol("fp.div") => {
      ...
      (ta(0).symbol.sort, ta(1).symbol.sort, ta(2).symbol.sort) match {
        case (RoundingModeSort, fp1 : FPSort, fp2 : FPSort) if (fp1 == fp2) =>
          uppsat.ast.AST(FloatingPointTheory.FPDivisionFactory(fp1,
                                                               fp1,
                                                               fp1),
                         ta.toList)

        case (s1, s2, s3) => {
          val msg = "Wrong sorts for fp.div: " + ((s1, s2, s3))
          throw new SMTParserException(msg)
        }
      }
    }
    ...
    case PlainSymbol("fp") => {
      ...
      val fpsort =
        uppsat.theory.FloatingPointTheory.FPSortFactory(List(eBits.length,
                                                             sBits.length+1))
      if (!eBits.contains(1) && !sBits.contains(1)) {
        ...
        val symbol =
          uppsat.theory.FloatingPointTheory.FloatingPointLiteral(signBit,
                                                                 eBits,
                                                                 sBits,
                                                                 fpsort)
        uppsat.ast.Leaf(symbol)
    ...
    case _ if ("to_fp_\\d+_\\d+".r.findFirstIn(asString(sym)).isDefined) => {
      ...
      asString(sym) match {
        case p(eBits, sBits) => {
          val targetSort = FloatingPointTheory.FPSortFactory(List(eBits.toInt,
                                                                  sBits.toInt))
    ...
    case _ if ("\\+oo".r.findFirstIn(asString(sym)).isDefined) => {
      ...
      asString(sym) match {
        case p(eBits, sBits) => {
          val sort =
            FloatingPointTheory.FPSortFactory(List(eBits.toInt, sBits.toInt))
    ...
    case _ if ("bv(\\d+)_(\\d+)".r.findFirstIn(asString(sym)).isDefined) => {
      ...
      asString(sym) match {
        case p(constant, bits) => {
          ...
          val symbol =
            uppsat.theory.BitVectorTheory.BitVectorLiteral(padding ++
                                                             constantBitList,
                                                           sort)
          uppsat.ast.Leaf(symbol)
        }
      ...
      // TODO: (Peter) This should be done more properly, probably with a
      // val-defined pattern.

      // TODO: (Aleks) Should it be "sign_extend_1" or "sign_extend 1" (notice
      // the underscore)
    ...
    case _ if ("extract".r.findFirstIn(asString(sym)).isDefined) => {
       ...
       (asString(sym)) match {
         case p(start, end) => {
           ...
           val value =
             BitVectorTheory.BVExtractFactory(start.toInt, end.toInt)(sort)
           uppsat.ast.AST(value, List(arg))
         }
       }
     }
    ...
    case PlainSymbol("concat") => {
      ...
      (l.symbol.sort, r.symbol.sort) match {
        ...
        case (s1, s2) =>
          throw new SMTParserException("Concat of non-BV: " + s1 + ", " + s2)
      }
    }
